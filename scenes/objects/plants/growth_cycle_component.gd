...
func growth_states(starting_day: int,current_day: int):
	if current_growth_state == DataType.GrowthStates.Maturity:
		return
	...
	var name = DataType.GrowthStates.keys()[current_growth_state]
...
func harvest_state(starting_day: int,current_day: int)-> void:
	if current_growth_state == DataType.GrowthStates.Harvesting:
		return
		
	var days_passed =(current_day-starting_day)% days_until_harvest
	
	if days_passed == days_until_harvest - 1:
		current_growth_state = DataType.GrowthStates.Harvesting
		crop_harvesting.emit()
		
func get_current_growth_state()-> DataType.GrowthStates:
